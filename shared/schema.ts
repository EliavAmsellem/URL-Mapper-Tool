...
export const mappingResults = pgTable(
  "mapping_results",
  {
    id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  ...
  (t) => [index("mapping_results_job_id_idx").on(t.jobId)],
);
...
export const translationCache = pgTable(
  ...
  (t) => [
    uniqueIndex("translation_cache_text_src_tgt_uniq").on(t.sourceText, t.sourceLang, t.targetLang),
  ],
);
